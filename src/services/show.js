...
export async function getByCinemaId(cinemaId) {
  ...
  const allScreensId = screenByCinemaIdInfor.length;
  const screenIdArr = [];
  for (let i = 0; i < allScreensId; i++) {
    screenIdArr.push(screenByCinemaIdInfor[i].id);
  }
  const showByScreenIdInfor = await show.getByMutipleScreenId(screenIdArr);
  return showByScreenIdInfor;
}
